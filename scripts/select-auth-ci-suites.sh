...
if [[ ${#changed_files[@]} -gt 0 ]]; then
    for changed_file in "${changed_files[@]}"; do
        case "$changed_file" in
            .github/workflows/auth-android-smoke.yml|scripts/resolve-nightly-apk.sh|scripts/select-auth-ci-suites.sh|scripts/test-hosted-flow-registration.sh|scripts/test-resolve-nightly-apk.sh|scripts/test-select-auth-ci-suites.sh|maestro/auth/smoke/*|maestro/auth/subflows/add-offline-account.yaml|maestro/auth/subflows/enter-offline-mode.yaml)
                full_matrix=true
                ;;
            maestro/auth/online/*)
                ;;
