...
if [[ ${#changed_files[@]} -gt 0 ]]; then
    for changed_file in "${changed_files[@]}"; do
        case "$changed_file" in
            maestro/auth/online/password-login.yaml|maestro/auth/online/prepared-recovery-password-reset.yaml|maestro/auth/online/prepared-totp-login-complete.yaml|maestro/auth/online/prepared-totp-login-start.yaml|maestro/auth/online/signup-recovery-login.yaml|maestro/auth/online/unknown-login.yaml|maestro/auth/subflows/add-online-code.yaml|scripts/current-totp.mjs)
            ...
            maestro/auth/online/prepared-bulk-mutation-complete.yaml|maestro/auth/online/prepared-bulk-mutation-start.yaml|maestro/auth/online/prepared-password-login.yaml)
                add_lane data-sync
                ;;
            .github/scripts/run-auth-online-tests.sh|.github/workflows/auth-android-online.yml|maestro/auth/subflows/assert-synced-code.yaml|maestro/auth/subflows/configure-online-test-endpoint.yaml|maestro/auth/subflows/dismiss-code-guidance.yaml|maestro/auth/subflows/login-online-account.yaml|museum/*|scripts/fixtures/*|scripts/resolve-auth-nightly.sh|scripts/select-auth-online-lanes.sh|scripts/test-select-auth-online-lanes.sh)
                full_matrix=true
                ;;
            # A new online flow must not silently receive partial coverage.
            maestro/auth/online/*.yaml)
                full_matrix=true
                ;;
        esac
    done
